...
/* ---- arrowhead geometry -------------------------------------------------- */
// Thin wrapper over the default ArrowEngine (src/arrows/), kept on router's public
// surface so callers/tests can stay on `arrowHeading(poly, …)`. The engine does the
// real work; `source`/`target` are vestigial (the heading comes from the line).
export function arrowHeading(poly, source, target, mode, back = 11) {
  return getArrowEngine("default").arrow({ poly }, { mode, back });
}
